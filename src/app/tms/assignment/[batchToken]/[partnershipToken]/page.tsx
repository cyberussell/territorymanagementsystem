...
export default async function PartnershipWorkspacePage({
  ...
  // partnership.territories only ever holds this batch's own territories (resolved from the
  // token above), so signing their maps with the service-role client can't cross congregations.
  const [territoryStructures, signedTerritories] = await Promise.all([
    Promise.all(partnership.territories.map((t) => getTerritoryStructure(supabase, partnership.congregation_id, t.id))),
    withSignedTerritoryMapUrls(supabase, partnership.territories),
  ])
  ...
  return (
    <PublisherWorkspaceApp
      ...
      initialWorkspace={{ ...partnership, territories: signedTerritories }}
