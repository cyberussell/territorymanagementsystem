...
// Fills one partnership's worth of units from the front of unitQueue, mutating it in place, and
// returns the flat record-id list for that partnership. maxPerPartnership counts UNITS
// (households), not raw records — a household of 3 people still only fills one slot. Units are
// taken strictly in the order given (already territory -> section -> block -> staleness, see
// fetchEligibleRecordIds), so every partnership except possibly the last one ends up with exactly
// maxPerPartnership units — a household is never split, but a block of records can be, if that's
// where the maxPerPartnership-sized slice happens to land. Takes at least one unit even if
// maxPerPartnership is 0 or negative, so a pathological cap can't stall progress. Shared by
// calculateAssignment (looping this once per partnership) and addPartnershipToBatch (queries.ts,
// filling exactly one newly-added partner from whatever's left over).
export function fillPartnershipFromUnits(unitQueue: string[][], maxPerPartnership: number): string[] {
  const takeCount = Math.max(1, maxPerPartnership)
  const taken = unitQueue.splice(0, takeCount)
  return taken.flat()
}
...
// only the last partnership may end up with fewer than the max.
export function calculateAssignment(
  ...
  for (let sequence = 1; sequence <= actualPartnershipCount; sequence += 1) {
    const recordIds = fillPartnershipFromUnits(unitQueue, maxPerPartnership)
